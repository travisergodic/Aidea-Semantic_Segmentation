...
class GaussianMask(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size, bandwidth, iskernel=True):
        ...
        kernel_weight = self._make_kernel_weight(in_channels, kernel_size, self.n_kernels)
...
class BilateralFilter(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, kernel_size, theta_alpha, theta_beta):
        ...
        kernel_weight = make_spatial_kernel(kernel_size, theta_alpha, isreshape=False)
    ...
    def forward(self, Q, I):
        ...
        #norm
        norm_weight = Ij * self.spatial_weight.squeeze(dim=2) #size B*C*N_ker*H*W
...
class CRFRNN(nn.Module):
    ...
    def __init__(self, n_iter, in_channels, n_classes, kernel_size=[3, 3],
        ...
        self.weightfiltering = Parameter(torch.rand(1, n_filters, n_classes, 1, 1))
